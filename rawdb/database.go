...
func NewDatabase(db keyvaluedb.KeyValueStore) keyvaluedb.Database {
	return &nofreezedb{
		KeyValueStore: db,
	}
}
...
func NewMemoryDatabase() keyvaluedb.Database {
...
func NewMemoryDatabaseWithCap(size int) keyvaluedb.Database {
	return NewDatabase(memorydb.NewWithCap(size))
}

// NewLevelDBDatabase creates a persistent key-value database without a freezer
// moving immutable chain segments into cold storage.
func NewLevelDBDatabase(file string, cache int, handles int, namespace string) (keyvaluedb.Database, error) {
...
func InspectDatabase(db keyvaluedb.Database) error {
	it := db.NewIterator()
	defer it.Release()
