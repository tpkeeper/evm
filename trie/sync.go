...
import (
	...
	"github.com/tpkeeper/evm/common/prque"
...
type Sync struct {
	database keyvaluedb.KeyValueReader // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database keyvaluedb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) Commit(dbw keyvaluedb.KeyValueWriter) (int, error) {
